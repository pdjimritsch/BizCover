...
[ImmutableObject(true)] public sealed partial class TokenStore : ITokenStore
{
    ...
    {
        if (Remove(key)) return Add(key, value);
    ...
    /// <param name="kvp"></param>
    /// <returns></returns>
    public bool Update(KeyValuePair<string, object> kvp)
    {
        if (Remove(kvp.Key)) return Add(kvp.Key, kvp.Value);
    ...
    /// <param name="values"></param>
    /// <returns></returns>
    public bool UpdateRange(string key, IEnumerable<object> values)
    {
        if (RemoveRange(key, values)) return Add(key, values);
    ...
    /// <param name="attributes"></param>
    /// <returns></returns>
    public bool UpdateRange(IDictionary<string, object> attributes)
    {
        if (RemoveRange(attributes.Keys))
        {
            foreach (var key in attributes.Keys)
            {
                if (!Update(key, attributes[key])) return false;
            }
